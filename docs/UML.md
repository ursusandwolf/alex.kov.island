...
- `Organism`: Base for all life in Nature domain. Optimized with primitive fallbacks and bound to Structure of Arrays (SoA) high-density storage using `AtomicLongArray` and `AtomicIntegerArray` (for health, age, and movement) for thread-safe concurrent access.
