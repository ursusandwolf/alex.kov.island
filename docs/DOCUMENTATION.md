...
- **Communication Infrastructure**: Implemented multiple independent networks:
    ...
    - **Pollution**: Air and water pollution are generated by industrial, power, and agricultural zones. Pollution spreads to neighbors and reduces residential desirability and happiness.
    - **Desirability**: Calculated for every tile based on infrastructure access (power, water, transport) and pollution penalties. High desirability is required for high-density growth.
    - **Zoning & Progression**: Buildings now support **Density levels** (Low/Medium/High). The `ZoningService` handles automatic upgrades when environmental conditions are met. 
    - **Wealth Tiers**: Residents progress through **Wealth tiers** (Poor/Middle/Wealthy) based on happiness and local desirability, influencing tax revenue.
- **Boundary Validation**: The domain includes a rigorous boundary test suite verifying economic limits (zero balance), spatial limits (grid corners/overflow), density (building collisions), social logic, electricity propagation, and **pollution dynamics**.
