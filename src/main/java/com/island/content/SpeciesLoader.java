...
public class SpeciesLoader {
    ...
    public SpeciesRegistry load() {
        ...
        for (SpeciesKey key : SpeciesKey.values()) {
            ...
            if (props.containsKey(code + ".weight")) {
                loadEntry(key, props, animalTypes, plantWeights, plantMaxCounts);
            }
        }

        return new SpeciesRegistry(animalTypes, plantWeights, plantMaxCounts);
    }
    ...
    private void loadEntry(SpeciesKey key, Properties props, 
        ...
        if (isPlant(key)) {
            ...
            int speed = Math.max(0, Integer.parseInt(props.getProperty(code + ".speed", "1")));
